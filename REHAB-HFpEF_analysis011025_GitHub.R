...
tree.rpart=rpart(outcome.diff~creatinine_mg_dl+troponin_i_pg_ml+hs_crp_mg_l+
                 ...
                 control=rpart.control(maxdepth = 2, minbucket = 25))
...
#   2.5 %   97.5 %
#   as.factor(sppb_node)2 0.1208125 1.557759
#   as.factor(sppb_node)3 1.7526599 3.118308
...
# -6.4355 -1.8393 -0.4355  2.1607  7.1607 
# 
# Coefficients:
#   Estimate Std. Error t value Pr(>|t|)    
# as.factor(sppb_node)2   0.8393     0.3628   2.314   0.0224 *  
#   as.factor(sppb_node)3   2.4355     0.3448   7.064 1.28e-10 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 2.715 on 116 degrees of freedom
# Multiple R-squared:  0.3227,	Adjusted R-squared:  0.311 
# F-statistic: 27.63 on 2 and 116 DF,  p-value: 1.537e-10
...
for (j in maxdepth){
  for (k in minbucket){
    ...
    for (i in 1:nfolds){
      ...
      #identify pairs in the train and test
      ...
      #get predictions for test set and store
      df=cbind(set.total,tree.leaves[[i]])
      colnames(df)
      tree.mod.train[[i]]=lm(outcome.diff ~ 0 + as.factor(`tree.leaves[[i]]`), data=df[df$pairin %in% pairs.train,])
      
      tree.train.rmse[[i]] <-  sqrt(mean((df[df$pairin %in% pairs.train,'outcome.diff']-
                                            predict(tree.mod.train[[i]],df[df$pairin %in% pairs.train,]))^2))
      tree.test.rmse[[i]] <-  sqrt(mean((df[df$pairin %in% pairs.test,'outcome.diff']-
                                           predict(tree.mod.train[[i]],df[df$pairin %in% pairs.test,]))^2))
      
      #this is the result from random forest
      rf.df.train=set.total[set.total$pairin %in% pairs.train,]
      rf.df.test=set.total[set.total$pairin %in% pairs.test,]
      colnames(rf.df.train) <- make.names(colnames(rf.df.train))
      colnames(rf.df.test) <- make.names(colnames(rf.df.test))
      
      rf.mod.train[[i]]=randomForest(outcome.diff~creatinine_mg_dl+troponin_i_pg_ml+hs_crp_mg_l+
                                       nt_pro_bnp+troponin_t_ng_l,
                                     data=rf.df.train, ntree=500)
      
      rf.train.rmse[[i]] <-  sqrt(mean((rf.df.train$outcome.diff-
                                          predict(rf.mod.train[[i]],rf.df.train))^2))
      
      rf.test.rmse[[i]] <-  sqrt(mean((rf.df.test$outcome.diff-
                                         predict(rf.mod.train[[i]],rf.df.test))^2))
      
    
    }  
    pdf(paste0("Matchingtree_SPPB_",nfolds,"fold_",j,"maxdepth_",k,"minbucket.pdf"), onefile = TRUE)
    for (i in 1:length(tree.2)) {
      rpart.plot(tree.2[[i]],box.palette = 'Reds',extra=1)
    }
    dev.off()
    print(paste0("maxdepth=",j,", minbucket=",k))
    print(paste0("mean training RMSE (DT):",mean(unlist(tree.train.rmse))))
    print(paste0("mean test RMSE (DT):",mean(unlist(tree.test.rmse))))
    
    print(paste0("mean training RMSE (RF):",mean(unlist(rf.train.rmse))))
    print(paste0("mean test RMSE (RF):",mean(unlist(rf.test.rmse))))
    ...
    print(paste0("lowest DT test RMSE:", bst))
    print(paste0("best setting minbucket=",bst.stg[2],", maxdepth=",bst.stg[1]))
    
    cvres.df[idx,]=c(j,k,mean(unlist(tree.test.rmse)),mean(unlist(rf.test.rmse)))
  }
}

#random forest model is same across runs, replace with average
cvres.df$testrmse.rf <- mean(cvres.df$testrmse.rf)
...
cvplot1 <- ggplot(data=cvres.df,mapping=aes(x=minbucket,color=maxdepth,fill=maxdepth))+
  geom_line(mapping=aes(y=testrmse.dt))+
  geom_point(mapping=aes(y=testrmse.dt,shape=maxdepth))+
  geom_line(mapping=aes(y=testrmse.rf),color='black',linetype='dashed')+
  ylab("Average Test RMSE")+
  labs(title="Average Test RMSE SPPB",
          caption="Random Forest Average Test RMSE (black dashed line)")+
  ylim(c(2.5,3.5))+scale_x_continuous(breaks=seq(5,30,5),n.breaks=5)

pdf("SuppFigure4_CVplot_SPPB.pdf",height=5,width=5)
cvplot1
dev.off()
setEPS()
postscript("SuppFigure4_CVplot_SPPB.eps")
cvplot1
dev.off()
...
tree.rpart=rpart(outcome.diff~creatinine_mg_dl+troponin_i_pg_ml+hs_crp_mg_l+
                   nt_pro_bnp+troponin_t_ng_l ,method="anova",data=set.total,
                 control=rpart.control(maxdepth=1,minbucket=25))
#this is the result after prune
set.total$outcome = set.total$fu_smw - set.total$bl_smw
tree.all=jesse.tree.select(tree.rpart,set.total)
...
# 2.5 %    97.5 %
# as.factor(smw_node)2 -1.04154  60.78633
# as.factor(smw_node)3 49.73780 108.49788
...
# Residuals:
#   Min      1Q  Median      3Q     Max 
# -215.64  -76.90    7.77   48.93  549.50 
# 
# Coefficients:
#   Estimate Std. Error t value Pr(>|t|)    
# as.factor(smw_node)2    29.87      15.61   1.914   0.0581 .  
# as.factor(smw_node)3    79.12      14.83   5.334 4.82e-07 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 116.8 on 116 degrees of freedom
# Multiple R-squared:  0.2168,	Adjusted R-squared:  0.2033 
# F-statistic: 16.06 on 2 and 116 DF,  p-value: 6.991e-07
...
for (j in maxdepth){
  for (k in minbucket){
    ...
    pdf(paste0("Matchingtree_6MWD_",nfolds,"fold_",j,"maxdepth_",k,"minbucket.pdf"), onefile = TRUE)
    for (i in 1:length(tree.2)) {
      rpart.plot(tree.2[[i]],box.palette = 'Reds',extra=1)
    }
    dev.off()
...
cvplot2 <- ggplot(data=cvres.df,mapping=aes(x=minbucket,color=maxdepth,fill=maxdepth))+
  geom_line(mapping=aes(y=testrmse.dt))+
  geom_point(mapping=aes(y=testrmse.dt,shape=maxdepth))+
  geom_line(mapping=aes(y=testrmse.rf),color='black',linetype='dashed')+
  ylab("Average Test RMSE")+
  labs(title="Average Test RMSE 6MWD",
       caption="Random Forest Average Test RMSE (black dashed line)")+
  ylim(c(105,130))+scale_x_continuous(breaks=seq(5,30,5),n.breaks=5)

pdf("SuppFigure4_CVplot_6MWD.pdf",height=5,width=5)
cvplot2
dev.off()
setEPS()
postscript("SuppFigure4_CVplot_6MWD.eps")
cvplot2
dev.off()
######################
